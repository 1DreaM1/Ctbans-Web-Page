...
<div class="center">
 ...
 <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search for SteamID, Admin, Reason.." title="Type in a SteamID"><br>
...
function show_ctban_table($dbh, $query, $results_per_page, $ctbanpage, $database, $table_prefix)
{
	...
	{	
		...
		$paginated_query = "SELECT * FROM " . $table_prefix . "ctbans ORDER BY id DESC LIMIT " . ($ctbanpage - 1) * $results_per_page . "," . $results_per_page;
		...
		print "<tr class='h'><th>SteamID</th><th>Admin SteamID</tah><th>Time</th><th>Duration</th>"
		...
		while ($row = mysqli_fetch_array($results))
		{
			...
			if ($row["length"] > 0)
			{
				$bantime = round($row["length"]/60, 1, PHP_ROUND_HALF_UP) . " min";
				if ($row["length"] >= 3600)
				{
					$bantime = round($row["length"]/3600, 0, PHP_ROUND_HALF_UP) . " hrs";
				}
				if ($row["length"] >= 86400)
				{
					$bantime = round($row["length"]/86400, 0, PHP_ROUND_HALF_UP) . " days";
				}
				
				if ($row["timeleft"] >= 0)
				{
					$timeleft = $row["timeleft"] . " sec";
				}	
				if ($row["timeleft"] >= 60)
				{
					$timeleft = round($row["timeleft"]/60, 1, PHP_ROUND_HALF_UP) . " min";
				}
				if ($row["timeleft"] >= 3600)
				{
					$timeleft = round($row["timeleft"]/3600, 1, PHP_ROUND_HALF_UP) . " hrs";
				}
				if ($row["timeleft"] >= 86400)
				{
					$timeleft = round($row["timeleft"]/86400, 1, PHP_ROUND_HALF_UP) . " days";
				}
				if ($row["removed"] == "E")
				{
					$timeleft = "Expired";
			...
			print "<tr><td class='" . $classtext ."'>" . $row["perp_steamid"]
				...
				. "</td><td class='" . $classtext . "'>" . date('r', $row["created"]) . "</td>" 
...
if (!$dbh) {
	...
	$query = "SELECT COUNT(*) FROM " . $table_prefix . "ctbans";
	...
	else
	{
		...
		show_ctban_table($dbh, $query, $results_per_page, $ctbanpage, $database, $table_prefix);
