...
if (DISPLAY_ALERT == true && $OUT_OF_DATE == true) {
	...
	print	'<div class="alert">
				...
				<center><strong><span style="float:left;">Upozornenie !</span></strong> Vaša verzia je zastaraná ! Pre update choďte <a href="https://github.com/1DreaM1/Ctbans-Web-Page"><b>Sem</b></a> !<center>
			</div>';
}
...
if (SIDEBAR_LOGIN == true) {
	if (isset($_SESSION['login_user'])){
			...
			$sid_admin = "<a onClick=".'"'."document.getElementById('add_admin').style.display='block'".'"'." id='admin_1'>Admini<i style='float:right;' class='material-icons' style='font-size:36px'>supervisor_account</i></a>
							<a onClick=".'"'."document.getElementById('add_ban').style.display='block'".'"'." id='admin_2'>Bany<i style='float:right;' class='material-icons' style='font-size:36px'>lock</i></a>";
		}
		else {
			$login_name = "Login";
...
<div id="id01" class="modal">
  ...
  <form class="modal-content animate" action="" method="post">
	...
	<h1 style="text-align:center;margin-bottom:35px;">Login</h1>
      ...
      <input type="text" placeholder="Zadajte meno" <?php if (isset($_COOKIE['member_ct_login'])) { echo "value='" . $_COOKIE['member_ct_login'] . "'"; } ?> name="uname" required>

      <label for="psw"><b>Heslo</b></label>
      <input type="password" placeholder="Zadajte heslo" <?php if (isset($_COOKIE['member_ct_login'])) { echo "value='" . $_COOKIE['member_ct_password'] . "'"; } ?> name="psw" required>
        
      <button class="button2" type="submit" name="login_btn">Login</button>
      <label>
        <input type="checkbox" <?php if(isset($_COOKIE["member_ct_login"])) { echo "checked='checked'"; } ?> name="remember"> Zapamätať si
      </label>
    </div>

    <div class="container2" style="background-color:#f1f1f1">
      <button type="button" onclick="document.getElementById('id01').style.display='none'" class="cancelbtn" style="width:20%;">Zatvoriť</button>
...
if (isset($_SESSION['login_user'])) {
	...
	print "<center>Ste prihlásený ako <b>" . $_SESSION['login_user'] . "</b>.</center>";
...
<div id="add_admin" class="modal" <?php if ((isset($_GET['adminid']) || isset($_POST['add_admin']) || isset($_POST['add_admin_btn'])) && isset($_SESSION['login_user'])) { echo "style='display:block;'"; } ?>>
  ...
  <form class="modal-content-admin animate" action="" method="post">
    ...
    <div class="container2">
	<h1 style="text-align:center;margin-bottom:35px;">Admini</h1>
	...
	if (isset($_GET['adminid']) && isset($_SESSION['login_user'])) {
		...
		elseif (isset($_POST['save']) && isset($_SESSION['login_user'])) {
			...
			echo "<center><b><span style='padding:10px;color:#33cc33;'>Uložené</span></b></center>";
		}
		else {
			...
			print '<table id="spec">' . "\n";
			print "<tr><td><b>Meno</b></td><td>" . $input_user . "</td></tr>\n";
			print "<tr><td><b>Heslo</b></td><td>" . $input_pass . "</td></tr>\n";
			print "</table>\n";
			echo "<br><br>";
		
			if (isset($_POST['edit']) && isset($_SESSION['login_user'])) {
				print "<table style='text-align:left;margin-left:auto;margin-right:auto;width:90%;'><tr><th><button type='submit' name='save' value='" . $row_add_admin['id'] . "' class='edit'><span style='font-size:14px;'>Uložiť</span></button></th></tr></table>";
			}
			else {
				
				if ($_GET['adminid'] == $_SESSION['login_id'] || $_SESSION['login_id'] <= 1) {
					print "<table style='text-align:left;margin-left:auto;margin-right:auto;width:90%;'><tr><th><button type='submit' name='edit' value='" . $row_add_admin['id'] . "' class='edit'><span style='font-size:14px;'>Upraviť</span></button> <button type='submit' name='delete' value='" . $row_add_admin['id'] . "' class='delete'><span style='font-size:14px;'>Odstrániť</span></button></th></tr></table>";
				}
				else {
					print "<center><b>Nemáte prístup k úpravám adminov.</b></center>";
				}
			}
		}
	}
	elseif (isset($_POST['add_admin']) && isset($_SESSION['login_user'])) {
		...
		print "<br>";
		print "<table id='spec'>\n";
		print "<tr><td><b>Meno</b></td><td><input type='text' name='uname' required></td></tr>\n";
		print "<tr><td><b>Heslo</b></td><td><input type='text' name='pswrd' required></td></tr>\n";
		print "</table>\n";
		print "<br><br>";
		print "<table style='text-align:left;margin-left:auto;margin-right:auto;width:90%;'><tr><th><button type='submit' name='add_admin_btn' class='button'><span style='font-size:18px;'>Uložiť</span></button></th></tr></table>";
		
	}
	elseif (isset($_POST['add_admin_btn']) && isset($_SESSION['login_user'])) {
			
		echo "<center><b><span style='padding:10px;color:#33cc33;'>Pridané</span></b></center>";
	}
	elseif (isset($_SESSION['login_user'])) {
	
		print '<table id="bans">' . "\n";
		print "<tr><th>ID</th><th>Meno</th><th>Heslo</th></tr>\n";
	
		while ($row_add_admin = mysqli_fetch_array($q_add_admin)) {
			
			if ($row_add_admin["id"] == $_SESSION['login_id'] || $_SESSION['login_id'] <= 1) {
		
				print "<tr title='UPRAVIŤ' onclick=" . '"' . "window.location = '{$_SERVER['PHP_SELF']}?adminid=" . $row_add_admin['id'] . "';" . '"'. ">
						<td>" . $row_add_admin["id"]. 
	...
	if (isset($_POST['delete']) && isset($_SESSION['login_user'])) {
		...
		if ($ad_id == 1) {
			echo "<center><b><span style='padding:10px;color:red;'>Hlavný účet nie je možné odstrániť !</span></b></center>";
		}
		else {
			mysqli_query($dbh, "DELETE FROM `CTBan_Login` WHERE `id`='$ad_id'");
			echo "<center><b><span style='padding:10px;color:#33cc33;'>Vymazané</span></b></center>";
			unset($_POST['delete']);
		}
	}
    ...
    <div class="container2" style="background-color:#f1f1f1">
      <button type="button" onclick="document.location='<?php echo $_SERVER['PHP_SELF']; ?>'" class="cancelbtn">Zatvoriť</button>
	  <?php if (isset($_POST['add_admin_btn']) || isset($_POST['add_admin']) || isset($_GET['adminid']) || $_SESSION['login_id'] > 1) { } else { echo '<form method="post"><button type="submit" name="add_admin" class="cancelbtn">Pridať</button></form>';} ?>
    </div>
  </form>
</div>
...
<div id="add_ban" class="modal" <?php if (isset($_POST['ban_save']) && isset($_SESSION['login_user'])) { echo "style='display:block;'"; } ?>>
  ...
  <div class="modal-content-admin animate">
    ...
    <div class="container2">
	<h1 style="text-align:center;margin-bottom:35px;">Bany</h1>
	...
	if (isset($_POST['ban_save']) && isset($_SESSION['login_user'])) {
		...
		print "<table id='spec'>\n";
		print "<tr><td><b>Meno</b></td><td><input type='text' name='name' required></td></tr>\n";
		print "<tr><td><b>SteamID</b></td><td><input type='text' name='sid' required></td></tr>\n";
		print "<tr><td><b>Dĺžka Banu</b></td><td><input type='text' name='length' required></td></tr>\n";
		...
		print "<tr><td><b>Dôvod</b></td><td><input type='text' name='reason' required></td></tr>\n";
		print "<tr><td><b>Zostávajúci čas</b></td><td><input type='text' name='remaining' required></td></tr>\n";
		print "</table>\n";
		print "<br><br>";
		print "<table style='text-align:left;margin-left:auto;margin-right:auto;width:90%;'><tr><th><button type='submit' name='ban_save' class='button'><span style='font-size:18px;'>Uložiť</span></button></th></tr></table>";
		print '</form>';
	}
...
function show_ctban_table($dbh, $query, $results_per_page, $ctbanpage, $table_prefix, $database)
{
	...
	{	
		...
		if ($ctbanpage != 1)
		{
			echo " <a class='pg'; href='{$_SERVER['PHP_SELF']}?ctbanpage=1'> Prvý</a> ";
			echo " ";
			$previouspage = $ctbanpage - 1;
			echo " <a class='pg'; href='{$_SERVER['PHP_SELF']}?ctbanpage=$previouspage'>&laquo; Predošlí</a> ";
		}
		if ($ctbanpage != $lastpage)
		{
			...
			echo " <a class='pg'; href='{$_SERVER['PHP_SELF']}?ctbanpage=$nextpage'>Ďalší &raquo;</a> ";
			echo " ";
			echo " <a class='pg'; href='{$_SERVER['PHP_SELF']}?ctbanpage=$lastpage'>Posledný</a> ";
		}
		echo "</p>";
		...
		print '<table id="bans">' . "\n";
		print "<tr class='h'><th>Meno</th><th>Čas Banu</th><th>Dĺžka</th>"
			. "<th>Meno Admina</th><th>Dôvod</th><th>Zostávajúci čas</th></tr>\n";
		...
		while ($row = mysqli_fetch_array($results))
		{
			...
			if ($row["bantime"] > 0)
			{
				...
				if ($row["bantime"] >=60) {
					$bantime = round($row["bantime"]/60, 1, PHP_ROUND_HALF_UP) . " hod";
				}
				elseif ($row["bantime"] >=1440) {
					$bantime = round($row["bantime"]/1440, 1, PHP_ROUND_HALF_UP) . " dní";
				}
				if ($row["timeleft"] >= 0)	{
					$timeleft = $row["timeleft"] . " min";
					
					if ($row["timeleft"] >=60) {
						$timeleft = round($row["timeleft"]/60, 1, PHP_ROUND_HALF_UP) . " hod";
					}
					elseif ($row["timeleft"] >=1440) {
						$timeleft = round($row["timeleft"]/1440, 1, PHP_ROUND_HALF_UP) . " dní";
					}
				}
				else	{
					$timeleft = "Expired";
			...
			print "<tr title='Podrobnosti' onclick=" . '"' . "window.location = '{$_SERVER['PHP_SELF']}?banid=" . $row['ban_id'] . "';" . '"'. ">
		...
		echo "<br>";
		if ($noresults==true) {
			print "<br><p><center><b><span style='color:red;font-size:16px;'>Žiadne výsledky</span></b></center></p><br><br><br><br><br>";
		}
		else {
			echo "<p>Strana " . strval($ctbanpage) . " z " . $lastpage . "<br><br>";
		
			$sufix = "";
			if ($rows == 1) {
				$sufix = "ban";
			}
			elseif ($rows > 1 && $rows <= 4) {
				$sufix = "bany";
			}
			else {
				$sufix = "banov";
	...
	if (!$dbh) {
		...
		if (!empty($_GET['banid'])) {
			...
			if (empty($row_q)) {
				...
				echo "<br><p><center><b><span style='color:red;font-size:16px;'>Žiadny výsledok (error 404)</span></b></center></p>";
				echo "<br><br><table style='text-align:center;margin-left:auto;margin-right:auto;width:90%;'><tr><th><button class='button' onclick=" . '"' . "window.location = '{$_SERVER['PHP_SELF']}';" . '"'. "><span style='font-size:18px;'>Späť</span></button></th></tr></table><br><br><br><br><br><br><br><br><br><br><br><br>";
			}
			else {
				...
				if ($row_q["bantime"] > 0)
				{
					...
					if ($row_q["bantime"] >=60) {
						$bantime = round($row_q["bantime"]/60, 1, PHP_ROUND_HALF_UP) . " hod";
					}
					elseif ($row_q["bantime"] >=1440) {
						$bantime = round($row_q["bantime"]/1440, 1, PHP_ROUND_HALF_UP) . " dní";
					}
					if ($row_q["timeleft"] >= 0)	{
						$timeleft = $row_q["timeleft"] . " min";
					
						if ($row_q["timeleft"] >=60) {
							$timeleft = round($row_q["timeleft"]/60, 1, PHP_ROUND_HALF_UP) . " hod";
						}
						elseif ($row_q["timeleft"] >=1440) {
							$timeleft = round($row_q["timeleft"]/1440, 1, PHP_ROUND_HALF_UP) . " dní";
						}
					}
					else	{
						$timeleft = "Expired";
				...
				print "<br><br>";
				print "<table style='text-align:left;margin-left:auto;margin-right:auto;width:90%;'><tr><th><h1><span>Hráč: " . utf8_encode($row_q['perp_name']) . "</span></h1></th></tr></table>";
				print "<br>";
				print "<table id='spec'>\n";
				print "<tr><td><b>Meno</b></td><td>" . $ban_name . "</td></tr>\n";
				print "<tr><td><b>SteamID</b></td><td>" . $ban_sid . "</td></tr>\n";
				print "<tr><td><b>Čas Banu</b></td><td>" . $ban_time . "</td></tr>\n";
				print "<tr><td><b>Dĺžka Banu</b></td><td>" . $ban_length . "</td></tr>\n";
				print "<tr><td><b>Admin</b></td><td>" . $ban_admin . "</td></tr>\n";
				print "<tr><td><b>Admin SteamID</b></td><td>" . $ban_admin_sid . "</td></tr>\n";
				print "<tr><td><b>Dôvod</b></td>" . $ban_reason . "</tr>\n";
				print "<tr><td><b>Zostávajúci čas</b></td>" . $ban_left . "</tr>\n";
				print "<tr><td><b>Počet banov</b></td><td>" . $bans_count[0] . "</td></tr>\n";
				print "</table>\n";
				print "<br><br>";
				...
				if (isset($_SESSION['login_user'])) {
					...
					if (isset($_POST['btn_edit'])) {
						print " <button class='btn_edit' name='btn_save' value='" . $row_q['ban_id'] . "'><span style='font-size:18px;'>Uložiť</span></button>";
					}
					else {
					print " <button class='btn_edit' name='btn_edit' value='" . $row_q['ban_id'] . "'><span style='font-size:18px;'>Upraviť</span></button>";
					print " <button class='btn_del' name='btn_del' value='" . $row_q['ban_id'] . "'><span style='font-size:18px;'>Vymazať</span></button>";
					}
				...
				if (isset($_POST['btn_del']) && isset($_SESSION['login_user'])) {
					...
					print '<div id="add_ban" class="modal" style="display:block;">
							...
							<div class="container2">
								<h1 style="text-align:center;margin-bottom:35px;">Bany</h1>
								<center><b><span style="padding:10px;color:#33cc33;">Vymazané</span></b></center>
							</div>

							<div class="container2" style="background-color:#f1f1f1">
								<button type="button" onclick="document.location=' . "'" . $_SERVER['PHP_SELF'] . "'" . '" class="cancelbtn">Zatvoriť</button>
							</div>
							</form>
							</div>';
				}
				...
				if (isset($_POST['btn_save']) && isset($_SESSION['login_user'])) {
					...
					print '<div id="add_ban" class="modal" style="display:block;">
							...
							<div class="container2">
								<h1 style="text-align:center;margin-bottom:35px;">Bany</h1>
								<center><b><span style="padding:10px;color:#33cc33;">Uložené</span></b></center>
							</div>
				...
				print "<table style='text-align:left;margin-left:auto;margin-right:auto;width:90%;'><tr><th><button class='button' onclick=" . '"' . "window.location = '{$_SERVER['PHP_SELF']}';" . '"'. "><span style='font-size:18px;'>Späť</span></button></th></tr></table>";
